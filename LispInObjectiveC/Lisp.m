...
- (id)objectForKey:(id)aKey {
    ...
    return result ? result : [self.outer objectForKey:aKey];
}
...
- (id)eval:(NSObject*)x env:(Env*)env {
    // Symbols are looked up in the environment
    if([x isKindOfClass:[NSString class]]) { return [env objectForKey:x]; }
    ...
    if([scrutinee isEqual:@"quote"]) {
        ...
        return [^(NSArray* args) {
            Env* newEnv = [[Env alloc] initWithOuterEnv:env];
        ...
        if([theBlock isKindOfClass:NSClassFromString(@"NSBlock")]) {
            ...
            NSLog(@"couldn't interpret: %@, env: %@", x, env);
        }
...
- (Env*)builtins {
    ...
    Env* builtins = [[Env alloc] initWithOuterEnv:nil];
